...
class Database(object):
    ...
    def fetch_data(self, start_id=1, end_id=-1):
        ...
        if not ret:
            # We relax the DM_NAME constraint in this case as some valid cases exist where
            # the equality is wrong. However, we still have invalid cases when the equality is not also checked.
            # refer to comments in FmkPlumbing.fmkdb_fetch_data()
            # TODO: find a better way than doing 2 sql queries
            stmt = \
                '''
                SELECT DATA.ID, DATA.CONTENT, DATA.TYPE, DMAKERS.NAME, DATA.DM_NAME, DATA.PRJ_NAME
                FROM DATA INNER JOIN DMAKERS
