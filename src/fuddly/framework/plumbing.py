    ...
    def _get_projects_fs(self, fmkDB_update=True):
        ...
        def populate_projects(search_path, prefix=""):
            ...
            for (path, dirs, files) in os.walk(search_path):
                rel_path=path.removeprefix(search_path)
                if "__init__.py" in files:
                    # Normapath remove the / that will most likely be at the end of prefix
                    #key=os.path.dirname(key)
                    key=os.path.normpath(os.path.join(
                            prefix,
                            os.path.dirname(rel_path).removeprefix("/"),
                        ))
                    basename=os.path.basename(path.removeprefix(search_path).removeprefix("/"))
                    print(basename)
                    if basename != "":
