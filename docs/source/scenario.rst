...
.. note:: All the examples (or similar ones) of this chapter are provided in the file
  ``<fuddly_root>/data_models/tutorial/tuto/strategy.py``.
...
- either in the data-model's python module (usually in a ``strategy.py`` file);
...
``tuto/prj.py`` (refer to :ref:`dm:mydf` for further explanation on file organization). Describing scenarios
...
In our example, the registration goes through the special object ``tactics`` (line 4) of ``tuto/strategy.py``
...
data model (refer to the file ``data_models/tutorial/tuto/strategy.py``). ``step1`` and ``step2`` are defined with
...
``tuto`` project (refer to the file ``projects/tuto/prj.py``), a mapping is created for the
scenario ``ex1``::

  project.map_targets_to_scenario('ex1', {0: 8, 1: 9, None: 9})

A mapping is a simple python dictionary that maps virtual target IDs to real target IDs. In our
case, virtual IDs 0 and 1 have been mapped respectively to real IDs 8 and 9. Finally, the last
...
.. note::
    It is described by the following code snippet extracted from ``data_models/tutorial/tuto/strategy.py``:
...
The following figure depicts the third call to the generator where the scenario run through
