...
The preferred way of running tool is through the fuddly cli, but they should also work by being called 
directly, though it is not guaranteed.
...
command will display a synthesis of what you want::
  ...
  fuddly tool fmkdb -i 4

And if you want to get all information, issue the following::

  fuddly tool fmkdb -i 4 --with-data --with-fbk
...
25th January 2016 (11:30) and 26th January 2016::
  ...
  fuddly tool fmkdb --info-by-date 2016/01/25-11:30 2016/01/26
...
Hereunder is shown the output of ``fuddly tool fmkdb -h``.

.. code-block:: none

    usage: fmkdb [-h] [--fmkdb PATH] [--no-color] [-v] [--page-width WIDTH]
                 ...
                 [-wa] [--without-fmkinfo] [--without-analysis] [--limit LIMIT] [--raw]
                 [-dd] [-df] [--data-atom ATOM_NAME] [--fbk-atom ATOM_NAME]
    ...
    Arguments for FmkDB toolkit script
    ...
    Configuration Handles:
      ...
      --project PROJECT_NAME
                            ...
                            Supported by: --info-by-date, --info-by-ids, --data-with-
                            impact, --data-without-fbk, --data-with-specific-fbk
      --fbk-status-formula STATUS_REF
                            ...
                            status value that will be checked). Supported by: --data-with-
                            impact
    ...
    Fuddly Database Information:
      ...
      -ids, --info-by-ids FIRST_DATA_ID LAST_DATA_ID
                            Display information on all the data included within the
                            specified data ID range
    ...
    Fuddly Decoding:
      ...
      --data-atom ATOM_NAME
                            Atom of the data model to be used for decoding the sent data.
    ...
    Fuddly Database Analysis:
      ...
      -a, --add-analysis IMPACT COMMENT
                            Add an impact analysis to a specific data ID (expect --data-
                            id). IMPACT should be either 0 (no impact) or 1 (impact), and
                            COMMENT provide information
      --disprove-impact FIRST_ID LAST_ID
                            Disprove the impact of a group of data present in the outcomes
                            of '--data-with-impact-raw'. The group is determined by
                            providing the smaller data ID (FIRST_ID) and the bigger data ID
                            (LAST_ID).
...
Similarly to ``fmkdb`` you can run it through the fuddly cli if fuddly has been installed (through a
package manager or in a venv). If not you can run it from the command line directly.
...
To do that, you can use the plotty CLI, at ``<root of fuddly>/tools/plotty/plotty.py``::
  ...
  fuddly tool plotty -ids '0..100|2'
...
For further information refer to the help by issuing::
  ...
  fuddly tool plotty -h


Plotty Manual
-------------

Hereunder is shown the output of ``fuddly tool plotty -h``

.. code-block:: none

      usage: plotty [-h] -ids DATA_IDS [-df DATE_FORMAT] [-db PATH [PATH ...]] [-f FORMULA] [-poi POINTS_OF_INTEREST] [-gm {all,poi,auto}] [-hp]
              [-l ANNOTATIONS [ANNOTATIONS ...]] [-al ASYNC_ANNOTATIONS [ASYNC_ANNOTATIONS ...]] [-o OTHER_DATA_IDS] [-s VERTICAL_SHIFT]
      ...
      Main parameters:
        -ids, --data_ids DATA_IDS
                              The data ids to take into account should be: either <id_start>..<id_stop>[|<step>], or <id_start_1>..<id_stop_1>[|<step_1>], ...,
      ...
      Display Options:
        ...
        -poi, --points-of-interest POINTS_OF_INTEREST
                              How many point of interest the plot should show. Default is none
        -gm, --grid-match {all,poi,auto}
      ...
      Labels Configuration:
        -l, --labels ANNOTATIONS [ANNOTATIONS ...]
                              Display the specified labels for each Data ID represented in the curve. ('t' for TYPE, 'g' for TARGET, 's' for SIZE, 'a' for
                              ACK_DATE)
        -al, --async-labels ASYNC_ANNOTATIONS [ASYNC_ANNOTATIONS ...]
                              Display the specified labels for each Async Data ID represented in the curve. ('i' for 'ID', 't' for TYPE, 'g' for TARGET, 's'
                              for SIZE)
      
      Multiple Curves Options:
        -o, --other-data_ids OTHER_DATA_IDS
                              Other data IDs to plot against the main one. All other options also apply to it
        -s, --vertical-shift VERTICAL_SHIFT
...
Given a simple command line::
  ...
  fuddly tool plotty -id '2..12' -l t -al t i -df "%M:%S.%f" -poi 3
